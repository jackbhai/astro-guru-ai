...
import {
  ...
  LampIcon, HeartIcon, RingsIcon, OmIcon,
} from './components/icons.jsx'

const SUGGESTIONS = [
  'Black hole kya hota hai?',
  'Aryabhata kaun the aur kya khoja?',
  'Chandrayaan-3 ne kya achieve kiya?',
  'Shani ke ring kis cheez ke bane hain?',
  'Mayan log Venus kaise track karte the?',
  'Big Bang ke proof kya hain?',
]
...
const GREETING = {
  ...
  content:
    ...
    '**Astronomy Chat**: space ke koi bhi sawaal pucho — black holes, ISRO, NASA, Aryabhata, Greek/Chinese/Mayan/Egyptian astronomy — main khud jawab banata hoon.\n\n' +
    '**Kundali & Prediction**: DOB daalo — Vedic + Western + Chinese BaZi + pandit checks (Manglik, Kaal Sarp, Sade Sati, yogas) sab milega.\n\n' +
    '**Kundali Milan**: do janmpatrika — ashtakoota 36 guna matching pandit style.\n\n' +
...
export default function App() {
  ...
  async function send(text) {
    ...
    try {
      if (engineState !== 'ready') {
        ...
        setMessages((m) => [...m, { id: loadMsgId, role: 'assistant', source: 'system', content: '**AI model load ho raha hai…** Pehli baar download hoga (Wi-Fi pe karo), phir cached rahega. Progress upar panel mein dikh raha hai.' }])
  ...
  const checkBadge = (ok, label) => (
    <span className={`badge-check ${ok ? 'ok' : 'bad'}`}>{label}</span>
  )
  ...
  return (
    <div className="app">
      ...
      <header className="header">
        <div className="brand">
          <span className="logo-orb"><MoonIcon size={22} sw={1.6} /></span>
          <div>
            <h1>Astro-Guru</h1>
            <p>Hinglish Astronomy + Jyotish AI · 100% Free · On-Device</p>
          </div>
        </div>
        <span className={`badge ${aiOn ? 'ai' : ''}`}>
          <CrystalIcon size={13} sw={2} /> {aiOn ? 'AI ON' : 'AI OFF'}
        </span>
      </header>
      ...
      <section className="panel">
        <div className="panel-row">
          <div className="model-picker">
            <label htmlFor="model">AI Model — browser ke andar chalta hai, koi server nahi</label>
          ...
          <button className="load-btn" onClick={handleLoadModel} disabled={engineState === 'loading' || aiOn}>
            {aiOn ? 'AI Ready' : engineState === 'loading' ? 'Loading…' : 'AI Mode ON Karo'}
          </button>
        </div>
        {engineState === 'loading' && (
        ...
        {!webgpuOk && (
          <div className="notice">
            <span className="notice-icon"><WarnIcon size={15} sw={2} /></span>
            Is browser mein WebGPU nahi hai, isliye AI yahan nahi chalega. Chrome ya Edge ka latest version kholo. Kundali calculations tab tak kaam karengi.
          </div>
        )}
        {engineState === 'error' && <div className="notice"><span className="notice-icon"><WarnIcon size={15} sw={2} /></span>{progress.text}</div>}
      </section>
      ...
      <nav className="tabs">
        ...
        <button className={tab === 'jyotish' ? 'active' : ''} onClick={() => setTab('jyotish')}>
          <CrystalIcon size={16} sw={1.7} /> Kundali &amp; Prediction
        </button>
        <button className={tab === 'match' ? 'active' : ''} onClick={() => setTab('match')}>
          <RingsIcon size={16} sw={1.7} /> Kundali Milan
        </button>
      </nav>
      ...
      {tab === 'chat' && (
        <>
          <main className="chat">
            {messages.map((m, i) => (
              <div key={m.id ?? i} className={`row ${m.role}`}>
                {m.role === 'assistant' && <span className="avatar"><MoonIcon size={20} sw={1.6} /></span>}
                <div className={`bubble ${m.role} ${m.source || ''}`}>{renderContent(m.content)}</div>
              </div>
            ))}
          ...
          <div className="chips">
            {SUGGESTIONS.map((s) => (
              <button key={s} onClick={() => send(s)} disabled={busy}>
                <StarIcon size={12} sw={1.6} /> {s}
              </button>
            ))}
          </div>
          <form className="inputbar" onSubmit={(e) => { e.preventDefault(); send() }}>
            <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Space/astronomy ke baare mein pucho (Hinglish mein)…" autoComplete="off" />
            <button type="submit" disabled={busy || !input.trim()} aria-label="Send"><SendIcon size={18} /></button>
          </form>
        </>
      )}

      {tab === 'jyotish' && (
        <main className="jyotish">
          <div className="card panchang-card">
            <h3><OmIcon size={17} sw={1.6} /> Aaj ka Panchang · {panchang.date}</h3>
          ...
          <div className="card form-card">
            <h3><LampIcon size={17} sw={1.6} /> Apni Kundali Banao — real planetary math</h3>
            <div className="grid">
              <div className="field">
                <label>Naam</label>
          ...
          {chartData && (
            <div className="card">
              <h3><ScrollIcon size={17} sw={1.6} /> {chartData.meta.name} ki Kundali · {chartData.meta.dob} · {chartData.meta.city}</h3>
              ...
              <h4><OrbitIcon size={15} sw={1.6} /> Graha Sthiti · Vedic/Sidereal · Lahiri {chartData.ayanamsa_lahiri}°</h4>
              ...
              <h4><HourglassIcon size={15} sw={1.6} /> Vimshottari Dasha</h4>
          ...
          {chartData?.pandit && (
            <div className="card">
              <h3><OmIcon size={17} sw={1.6} /> Pandit Checks — dosha / yoga / gochara</h3>
              ...
              {chartData.pandit.lucky && (
                <p className="lucky-strip">
                  ...
                  {' · '}<b>Guru gochar:</b> {chartData.pandit.gochara.guru_gochar.rashi} ({chartData.pandit.gochara.guru_gochar.benefic ? 'shubh house' : 'mixed'})
                  {' · '}<b>Shani:</b> {chartData.pandit.gochara.shani_gochar.rashi}
                </p>
              )}
              {chartData.pandit.yogas.length > 0 && (
                <>
                  <h4><SparkIcon size={15} sw={1.6} /> Yogas detected</h4>
          ...
          {chartData?.d9 && (
            <div className="card">
              <h3><StarIcon size={17} sw={1.6} /> D9 Navamsha — bhagya/partnership ka chart</h3>
              <p className="hint">D9 lagna: <b>{chartData.d9.lagna}</b> · Vargottama grah extra strong hote hain</p>
              <div className="table-wrap">
                <table className="graha-table">
          ...
          {chartData?.bazi && (
            <div className="card">
              <h3><LampIcon size={17} sw={1.6} /> BaZi — Chinese Four Pillars</h3>
              ...
              <p className="hint">
                Day Master: <b>{chartData.bazi.day_master}</b> · strongest element: <b>{chartData.bazi.strongest}</b> · weakest: <b>{chartData.bazi.weakest}</b>
                <br />{chartData.bazi.approx_note}
              </p>
            </div>
          )}
          ...
          {chartData && (
            <div className="card">
              <h3><SparkIcon size={17} sw={1.6} /> AI Reading — saari traditions + pandit checks combine</h3>
              <div className="chips">
                {QUICK_READINGS.map((qr) => (
                  <button key={qr} onClick={() => { setQuestion(qr); getReading(qr) }} disabled={readingBusy}>
                    <StarIcon size={12} sw={1.6} /> {qr}
                  </button>
                ))}
              </div>
              <div className="field question-field">
                ...
                <div className="inputbar">
                  ...
                  <button onClick={() => getReading()} disabled={readingBusy || !question.trim()} aria-lhidden="1"><CrystalIcon size={18} /></button>
                </div>
              </div>
      ...
      {tab === 'match' && (
        <main className="jyotish">
          <div className="card form-card">
            <h3><RingsIcon size={17} sw={1.6} /> Kundali Milan — Ashtakoota 36 Guna (pandit style)</h3>
            <div className="match-grid">
              <MatchForm title="Ladka / Person A" value={mA} onChange={setMA} />
              <MatchForm title="Ladki / Person B" value={mB} onChange={setMB} />
            </div>
            <button className="compute-btn" onClick={handleMatch}><HeartIcon size={15} sw={1.6} /> Milan Compute Karo</button>
            {matchErr && <div className="notice"><span className="notice-icon"><WarnIcon size={15} sw={2} /></span>{matchErr}</div>}
            <small className="hint">Varna · Vashya · Tara · Yoni · Maitri · Gana · Bhakoot · Nadi — classical tables, PyJHora-grade rules</small>
          </div>

          {match && (
            <div className="card">
              <h3><HeartIcon size={17} sw={1.6} /> {match.a.meta.name} × {match.b.meta.name} — Milan Report</h3>
              ...
              {match.koota.doshas.length > 0 && (
                <div className="checks-grid">
                  ...
                  {match.a.pandit?.manglik?.manglik && match.b.pandit?.manglik?.manglik && <span className="badge-check ok">Manglik × Manglik = neutralized (classic)</span>}
                </div>
              )}
              <div className="table-wrap">
              ...
              <button className="load-btn" onClick={getMatchReading} disabled={matchBusy}>
      ...
      <footer className="footer">
        Real calculations · AI 100% on-device · Predictions jyotish parampara ka interpretation hain — guidance ke liye, scientific guarantee nahi
      </footer>
    </div>
  )
}
